    ...
    $.extend({
    	...
    	table: {
            ...
            queryParams: function(params) {
            	return {
        			// 传递参数查询参数
        			pageSize:       params.limit ? params.limit : 100,
        			pageNum:        params.offset ? params.offset / params.limit + 1 : 1,
        ...
        operate: {
            ...
            // 删除信息
            remove: function(id) {
            	$.modal.confirm("确定删除该" + $.table._option.modalName + "信息吗？", function() {
			...
			// 添加WebService
			addWebService: function(id) {
				var url = $.common.isEmpty(id) ? $.table._option.createWsUrl : $.table._option.createWsUrl.replace("{id}", id);
				$.modal.open("添加WebService服务", url);
			},
			// 添加Hessian
			addHessian: function(id) {
				var url = $.common.isEmpty(id) ? $.table._option.createHsUrl : $.table._option.createHsUrl.replace("{id}", id);
				$.modal.open("添加Hessian服务", url);
			},
			// 发送消息
			sendMq: function(id) {
				var url = $.common.isEmpty(id) ? $.table._option.createMqUrl : $.table._option.createMqUrl.replace("{id}", id);
				$.modal.open("发送消息", url);
			},
            // 工具栏表格树修改
            editTree: function() {
